...
同页下方「业务开关」是纯文本配置项，改动后立即生效：

| 配置项 | 默认 | 说明 |
|---|---|---|
| `enable_asr` | `true` | 关掉后无字幕视频直接失败，不再走语音识别 |
| `share_summary_across_users` | `true` | 跨用户复用已总结的视频，见「成本控制」 |
| `registration_open` | `true` | 关闭后邀请码注册入口不可用 |
| `site_notice` | 空 | 前台公告文案 |
...
- **跨用户复用** —— 别人已经总结过同一个视频（按 `vid` 匹配）时，
  直接复制一份「转写 + 总结」快照，连下载音频都不用，详见下节

### 两种「额度」不要混淆

| | 谁在限制 | 超了报什么 | 怎么恢复 |
|---|---|---|---|
| **本站月度额度** | 本系统，按用户记账（Redis + `usage_monthly`） | `本月语音识别额度不足（剩余约 N 分钟）` | 等次月重置，或管理员在后台改 `asr_monthly_quota_sec` |
| **腾讯云账号额度** | 腾讯云，按账号计费 | `腾讯云账号的语音识别资源包/额度已耗尽` | 去腾讯云控制台买资源包或开通后付费 |

第二条和用户在本站用了多少**完全无关**：全新部署、一次 ASR 都没跑过也会出现，
常见原因是账号没买资源包、免费额度已过期、账号未实名，或用的是子账号密钥。

因为腾讯云的拒绝发生在「已经下载并上传完音频」之后，白白浪费几十秒和一份 COS 流量，
所以拿到该错误码后会在 Redis 里记一个 **10 分钟熔断**（`bsum:asr_quota_blocked`），
期间再提交会在下载之前就失败。管理员补好额度后，点一下后台的
「测试语音识别」即可立即清除熔断，不必等 TTL。

### 跨用户复用

`videos` 表按 `(user_id, vid)` 唯一，每个用户各有一行，但**内容**可以来自别人：

- 提交时若站内已有同一 `vid` 且带转写的记录，就把
  `transcript` / `transcript_source` / `summary` 复制进自己的新行
  （`copied_from_video_id` 记下来源，便于审计）
- 复制的是**快照**，不是共用同一行 —— 所以「重新总结」「删除」只影响自己那份
- 只有转写、没有总结的来源也会被复用：能省掉 ASR，AI 总结仍会重新跑
- 封面、COS 对象键、Cookie 一概不复制（共用对象键会导致一方删除把另一方的图也删掉）
- 后台「业务开关」里的 `share_summary_across_users` 可关闭（默认 `true`）

> 隐私提醒：开启后，A 用自己 Cookie 转写出来的内容会流向没有同等权限的 B。
> 私有小圈子部署一般没问题；若站点面向陌生人，建议关掉。
...
是否分段不再由固定字数阈值决定，而是由「单次调用需要多少输出预算」决定
（`summarizer.single_shot_fits`）：

| 输入长度 | 策略 | 原因 |
|---|---|---|
| **≤ 约 20000 字**（≈55 分钟语音） | **单次直连** | 模型看得到全文，不经过中间压缩，质量最高 |
| 超过 | 两/三级 Map-Reduce | 预算会顶到接口上限，只能分段 |

单次直连是默认路径，也是质量最好的路径。阈值由两个实测值推出：

- 中文约 `2.1` 字符/token（线上模型实测 1,000,000 字符 = 476,220 tokens）
- 推理模型的思考量随输入增长，约 `4` 倍输入 token
  （实测 6500 字符的分段思考了 5,471 tokens）
- 单次 `max_tokens` 上限取实测可用的 `65536`

**为什么废弃原来的固定阈值（12000/20000）**：那条 13803 字的视频走分段后
只产出 20% 的篇幅 —— 分段阶段的预算写死为 5596，而 6500 字的分段光思考就要
5471 tokens，正文一个都分不到（实测 `finish=length`、`reasoning=5595`、输出 0 字）。
而且这种空产出**不会抛异常**，会被原样拼进最终汇总，约 94% 的内容被静默丢弃。

现在分段的每一步都有产出守卫（`_require_output`）：产出低于 80 字视为失败，
先自动用接口上限重试，仍失败就明确报错 —— 宁可让用户重试，也不给一份
悄悄缺内容的总结。

- `chunk_size=6500`，`overlap=600`（仅分段时使用）
- 温度：分段 `0.3` / 汇总 `0.4`
- 输出比例 `35%`，下限 1000 字，上限 8000 字
- 各阶段 `max_tokens` 均按输入长度计算并封顶在 65536
